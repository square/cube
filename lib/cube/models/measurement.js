...
function Measurement(expression, start, stop, tier){
  ...
  this.isPyramidal = reduces[expression.reduce].pyramidal;
...
Measurement.prototype.measure = function measure(db, options, callback) {
  var _this = this;
  compute[this.flavor].call(this, db, options, function(time, value, values){ callback(new Metric(time, value, _this, values)); });
};
...
function unary(db, options, callback) {
  ...
  function task() {
    findOrComputeUnary.call(self, db, options, function(time, value, values) {
      callback(time, value, values);
      if (!--remaining) {
        self.emit('complete');
...
function findOrComputeUnary(db, options, callback) {
  ...
  function find(measurement, callback) {
    ...
    function foundMetrics(error, metric) {
      handle(error);
      if (metric) {
        callback(metric.time, metric.value, metric.values);  // send back value for this timeslot
    ...
    function computePyramidal(start, stop) {
      ...
      find(measurement, function(time, value, values) {
        ...
        if (!--bin.size) {
          callback(time, reduce(bin.values), bin.values);
          delete bins[time];
        }
      });
    }
    ...
    function computeFlat(start, stop) {
      ...
      Event.find(db, measurement, function(error, event) {
        ...
        if (event) {
          ...
          if (time < then) {
            callback(time, (values.length ? reduce(values) : reduce.empty), values);
