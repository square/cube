...
Measurement.prototype.measure = function measure(db, options, callback) {
  ...
  if (this.isGrouped && !Array.isArray(this.isGrouped.groups)) {
    Event.groups(db, this, function(error, group_names){
      ...
      compute[_this.flavor].call(_this, db, options, callback);
    });
  } else {
...
function unary(db, options, callback) {
  ...
  function task() {
    ...
    function nextGroup(prev_group){
      ...
      findOrComputeUnary.call(query_measurement, db, options, function(metric){
        onMetric(metric);
    ...
    if (self.expression.group) {
      ...
      findOrComputeUnary.call(self, db, options, onMetric);
    }
  }
}
...
function binary(db, options, callback) {
  ...
  if(this.isGrouped){
    ...
    other_measurement.on("complete", function(){
      ...
      grouped_measurement.measure(db, options, measure(grouped_measurement, other_measurement));
    });
    grouped_measurement.on("complete", complete(grouped_measurement, other_measurement));
    other_measurement.measure(db, options, measure(other_measurement, grouped_measurement));
  } else {
    left.on("complete", complete(left, right));
    right.on("complete", complete(right, left));

    left.measure(db, options, measure(left, right));
    right.measure(db, options, measure(right, left));
  }
};
