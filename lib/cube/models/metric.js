...
function find(db, measurement, callback){
  ...
  db.metrics(type, function(error, collection){
    ...
    if (measurement.group) query["_id.g"] = measurement.group;
...
function from_wire(row, measurement){
  ...
  if(!measurement.isPyramidal) {
    values = (row.vs || []).reduce(function(expanded, value){
  ...
  if(row._id.g) data.group = row._id.g;
...
function to_wire(){
  ...
  if(!this.measurement.isPyramidal){
    values = this.values.reduce(function(values, value){
  ...
  id = { e: this.e, l: this.l, t: this.time };
  if(this.group) id.g = this.group;
