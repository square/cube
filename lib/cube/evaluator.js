...
exports.register = function(db, endpoints) {
  var event        = require("./event").getter(db),
      event_putter = require("./event").putter(db),
  ...
  endpoints.http.push(
    ...
    endpoint("DELETE", "/1.0/metric",         metricRefresh),
    endpoint("GET",    "/1.0/metric/refresh", metricRefresh),
    endpoint("GET",    "/1.0/types",          typesGet),
    endpoint("GET",    "/1.0/types/get",      typesGet)
  );

  function metricRefresh(request, response) {
    request = url.parse(request.url, true).query;

    if (! (request.start && request.stop && request.type)) {
      metalog.info('cube_request', {is: 'error', error: 'specify type, start and stop'});
      ...
      metalog.info('cube_request', {is: 'refresh', type: type, start: start, stop: stop});

      event_putter.invalidate_range(type, start, stop);
