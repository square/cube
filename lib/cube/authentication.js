...
authentication.authenticator = function(strategy, db){
  metalog.minor('cube_auth', { strategy: strategy });
  return authentication[strategy](db);
};
...
authentication.mongo_cookie = function(db){
  var users;

  db.collection(options.get('mongodb')["authentication-collection"] || "users", function(error, clxn){
