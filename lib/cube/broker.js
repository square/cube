...
function Worker(qname, interval){
  ...
  this.add = function add(mbox, perform, args, on_complete) {
  ...
  function invoke(job) {
    ...
    job.args.push(function _completed(){
      ...
      metalog.minor('q_worker', {is: '<-!', mbox: job.mbox, am: self.report(), result: util.inspect(result).slice(0,80) });
      if (job !== active) metalog.warn('q_worker', {is: 'ERR', am: qname, error: 'job was missing when callback triggered', self: self.report() });
      active = null;
      job.complete(result);
    });
    // start the task
    metalog.minor('q_worker', {is: '?->', mbox: job.mbox, am: self.report(), perform_args: util.inspect(job.args).slice(0,80) });
  ...
  this.start = function start(){
    ...
    clock = setInterval(work.bind(self), interval);
  }
  ...
  function work (){
    ...
    else if (job = next()){ invoke(job); }
    else                  { self.onIdle();    }
  };


  function size(){  return _.size(queue); }
  function state(){ return ((clock === null) ? 'stop' : (active ? 'busy' : 'idle')); }
  ...
  function report(){   return { qname: this.qname, size: this.size, state: this.state, queue: _.keys(queue) }; };
  function toString(){ return util.inspect(this.report()); };

  Object.defineProperties(this, {
    size:   {get:   size},   state:    {get:   state},
    report: {value: report}, toString: {value: toString},
...
Job.prototype.complete = function(result){
  var on_completes = this.on_completes;
  for (var ii in on_completes){
    process.nextTick(function(){ on_completes[ii].apply(null, result); });
  };
}
...
Job.prototype.report   = function (){ return { mbox: this.mbox, args: this.args, on_completes: this.on_completes.length }; }
