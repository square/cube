...
module.exports = function(kind, db) {
  var options = config.get(kind),
      server = {},
  ...
  if (!options) {
    throw new Error('The provided type of server does not exist in the configuration file.');
  }
  ...
  server.start = function(server_start_cb) {
    ...
    function ready(db) {
      ...
      authenticator = authentication.authenticator(options["authenticator"], db);
  ...
  function try_close(name, obj){ if (obj){
    try {
      metalog.info(name+'_stopping', config.values);
