...
var db_options = { native_parser: true };
...
module.exports = function(options) {
      ...
      file.serve(request, response, function(error) {
        if (error) {
          metalog.event('cube_request',
                        { is: 'failed', msg: error, code: error.status, ip: request.connection.remoteAddress, path: u.pathname });
          response.writeHead(error.status, {"Content-Type": "text/plain"});
          response.end(error.status + "");
        } else {
          metalog.event('cube_request',
                        { is: 'static', ip: request.connection.remoteAddress, path: u.pathname },
                        'minor');
        }
      });
    });
  ...
  server.start = function(server_start_cb) {
    ...
    db.open(function(error) {
      ...
      if (! options["mongo-username"]) return ready();
  ...
  primary.on(  "close", function(){ metalog.info('cube_life', {is: 'http_close' }); });
  secondary.on("close", function(){ metalog.info('cube_life', {is: 'ws_close'   }); });
  function try_close(name, obj){ if (obj){
    try {
      metalog.info('cube_life', {is:(name+'_stopping'), options: options});
      obj.close( function(){ metalog.info('cube_life', {is:(name+'_stop')}); } );
    } catch(err){}
  } }
  server.stop = function(cb){
    ...
    event.stop();
    // stop serving
    try_close('http', primary);
    try_close('ws',   secondary);
    try_close('udp',  udp);
    // in a short while, stop db'ing
    setTimeout(function(){ try_close('mongo', db); }, 100);
    // finally, holler at your boys
