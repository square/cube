...
exports.getter = function(db) {
  ...
  function getter(request, callback) {
    ...
    measurement.measure((("id" in request) ?
  ...
  Measurement.prototype.measure = function measure(callback) {
    switch(this.flavor()){
      ...
      default: throw('cannot measure '+this.inspect());
    }
  }
  ...
  Measurement.prototype.inspect = function(){
    return { start: this.tier.bin(this.start), stop: this.tier.bin(this.stop), tier: this.tier.key, flavor: this.flavor(), expr: this.expression.source };
  }
  ...
  Measurement.prototype.unary = function unary(callback) {
    ...
    // metalog.inspectify(this.inspect());
    var remaining = 0,
        time0 = Date.now(),
...
function handle(error) {
  if (!error) return;
  metalog.info('cube_request', {is: 'metric error', error: error });
  throw error;
}
