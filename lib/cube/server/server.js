...
module.exports = function(options) {
  ...
  server.start = function() {
    ...
    db.open(function(error) {
      if (!error && options["mongo-username"]){
        username = options["mongo-username"];
        util.log("authenticating mongo user: " + username);
        password = options["mongo-password"];
        db.authenticate(username, password, function(err, success){
          if(success){
            util.log("mongo user authenticated");
            cb(null);
          }
          else {
            cb(err ? err : new Error('Could not authenticate user ' + username), null);
          }
        });
      }else{
        cb(error);
