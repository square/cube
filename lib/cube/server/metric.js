...
exports.getter = function(db) {
  ...
  // Pull the next task off the queue and start it.
  function nextTask() {
    if (currentTask = queuedTasks.shift()) currentTask();
  }
  ...
  return function(request, callback) {
    ...
    var remaining = 0, time = start;
    while (time < stop) ++remaining, time = tier.step(time);

    // Create an adapter that converts callback arguments to an object.
    var adapt = "id" in request ? adaptValueId(request.id) : adaptValue;

    // Add a task to the queue to find or compute the desired metric.
    // The task is done when all expected results have been returned.
    queuedTasks.push(function() {
      measure(expression, start, stop, tier, function(time, value) {
        callback(adapt(time, value));
        if (!--remaining) {
    ...
    // Tell the queue processor to wake up on the next tick.
    if (!currentTask) process.nextTick(nextTask);
  };
};

function adaptValueId(id) {
  return function(time, value) {
    return {time: time, value: value, id: id};
  };
}

function adaptValue(time, value) {
