...
// 2012-08-10 04:25:55.736
function timestamp() {
  function pad(n){ return n < 10 ? '0' + n.toString(10) : n.toString(10); }
  var d = new Date();
  var time = [pad(d.getUTCHours()), pad(d.getUTCMinutes()), pad(d.getUTCSeconds())].join(':');
  var date = [d.getUTCFullYear(),   pad(d.getUTCMonth()+1), pad(d.getUTCDate())   ].join('-');
  return [date, ' ', time, '.', d.getUTCMilliseconds()].join('');
}

var metalog = {
  putter:  null,
  log:     function(msg){ process.stderr.write(timestamp() + ' - ' + msg.toString() + "\n"); },
  silent:  function(){ }
};


var tracing = false; // false to mute tracing
...
metalog.inspectify = function inspectify(args){
  for (var idx in arguments) {
    process.stderr.write(idx + ": ");
    var val = arguments[idx];
    if (_.isFunction(val)) val = val.toString().slice(0, 80);
    process.stderr.write(util.inspect(val, false, 2, true)+"\n"); // , null, true
  }
