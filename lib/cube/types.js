  ...
  return function(type) {
    ...
    if (!collection) {
      ...
      db.collection(type + "_events", function(error, events) {
        var orig_find = events.find;
        events.find = function(){
          metalog.minor('cube_query', {is: 'events_find', query: arguments});
          return orig_find.apply(this, arguments);
        };
        collection.events = events;
      });
      db.collection(type + "_metrics", function(error, metrics) {
        var orig_find = metrics.find;
        metrics.find = function(){
          metalog.minor('cube_query', {is: 'metrics_find', query: arguments});
          return orig_find.apply(this, arguments);
        };
