...
exports.putter = function(db){
  ...
  function invalidate_range(type, start, stop) {
    var metrics = collection(type).metrics;
    for (var tier in tiers) {
      tierStart = tiers[tier].floor(start);
      tierStop  = tiers[tier].floor(stop);
      metalog.info("cube_compute", { is: "inval", type: type, tier: tier, times: [tierStart, tierStop], start: start, stop: stop });
      metrics.update({
        i: false,
        "_id.l": +tier,
        "_id.t": {$gte: tierStart, $lt: tierStop},
      }, invalidate, multi);
    }
  }
  putter.invalidate_range = invalidate_range;
  ...
  flushers.push(setInterval(function() {
    for (var type in timesToInvalidateByTierByType) {
      var metrics = collection(type).metrics,
          timesToInvalidateByTier = timesToInvalidateByTierByType[type];
      for (var tier in tiers) {
        metalog.info("cube_compute", { is: "flush", type: type, tier: tier, times: timesToInvalidateByTier[tier] });
        metrics.update({
          i: false,
          "_id.l": +tier,
