...
var metric_options = options["mongo-metrics"],
  ...
  this.flush = function(collection){
    _.each(type_tsets, function(type_tset, type){
      ...
      _.each(type_tset, function(tset, tier){
        ...
        metrics.update({ i: false, "_id.l": +tier, "_id.t": {$in: times}}, invalidate, multi);
      });
    });
  };
...
exports.putter = function(db){
  ...
  function save(type, event) {
    ...
    invalidator.add(type, event);
    if (event.on_save) event.on_save(null);
  }
