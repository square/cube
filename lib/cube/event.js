...
var _ = require("underscore"),
    tiers    = require("./tiers"),
    models   = require("./models"), Event = models.Event, Metric = models.Metric,
...
exports.stop = function(){ Invalidator.stop_flushers(); };
...
function Invalidator(){
  ...
  this.add = function(type, ev){
    ...
    for (var tier in tiers){ tt[tier][tier*Math.floor(ev.t/tier)] = true; }
  };
    ...
    _.each(type_tsets, function(type_tset, type){
      db.metrics(type, function(error, collection){
        ...
        _.each(type_tset, function(tset, tier){
          ...
          metalog.info("cube_compute", { is: "flush", type: type, tier: tier, times: times });
  ...
  function type_tset(type){
    if (! (type in type_tsets)) type_tsets[type] = empty_tsets();
    return type_tsets[type];
  }
  function empty_tsets(){ return _.mapHash(tiers, function(){ return {}; }); }
...
Invalidator.start_flusher = function(cb){ Invalidator.flushers.push(setInterval(cb, invalidateInterval)); };
Invalidator.stop_flushers = function(){   Invalidator.flushers.forEach(clearInterval); Invalidator.flushers = []; };

var invalidator = new Invalidator();
exports.invalidator = function(){ return invalidator; };
...
exports.putter = function(db){
  ...
  function putter(request, callback) {
    ...
    // // Drop events from before invalidation horizon
    // if ((! request.force) && (time < new Date(new Date() - options.horizons.invalidation))) {
    //   metric.info('cube_compute', {error: "event before invalidation horizon"});
    ...
    if (type in knownByType) return event.save(after_save);
    ...
    event.save(function(error, event){
      after_save(error, event); // trigger normal callback

      knownByType[type] = true;
      eventsToSaveByType[type].forEach(function(event) { event.save(after_save); });
      delete eventsToSaveByType[type];
    });

    // After saving the event, queue invalidation of any cached
    ...
    function after_save(error, event) {
      // metalog.minor("cube_request", { is: "ev", at: "save", type: type, event: event });
      handle(error);
      invalidator.add(event._type(), event);
  ...
  Invalidator.start_flusher(function(){
