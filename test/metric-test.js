...
function gen_request(attrs){
  var req = { start: t1, stop: t2, step: units.second10, expression: 'sum(test)'};
...
suite.addBatch(test_helper.batch({
  topic: function(test_db){
    ...
    for (var i = 0; i < 250; i++){
      put_queue.defer(function(cb){
        putter({ type: "test", time: gen_date(i*10).toISOString(), data: {i: i}}, function(){ cb(null, null); });
      });
    }
  ...
  'with request id' : {
    ...
    'sends a null metric for the end slot': function(results){ assert.deepEqual(results[4][0].report(), {id: 'joe', time: gen_date(40), value: null}); }
  },
  'simple constant' : {
    topic:   function(getter){
      var checker = assert.isCalledTimes(this, 5);
...
//     for (var i = 0; i < 250; i++){
//       put_queue.defer(function(cb){
//         putter({ type: "test", time: gen_date(i * 3).toISOString(), data: {i: 3 * i}}, function(){ cb(null, null); });
//       });
//     }
//     test_helper.inspectify('d', arguments);
//     put_queue.await(function(){ callback(null, getter) });
//   },
//   'hi': {
//     topic: function(getter){
//       test_helper.inspectify('3', arguments);
//       this.ret = getter(gen_request({start: t1, stop: t2, expression: 'sum(test)'}), this.callback);
//     },
//     'fires callback with id': function(result, j_){
//       test_helper.inspectify('f', arguments);
//       // assert.equal(result.value, (result.time > nowish_floor ? undefined : 0));
//       // assert.include([nowish_floor, 10e3+nowish_floor], +result.time);
